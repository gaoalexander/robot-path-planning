...
class Game:
    ...
    def on_click(self, event):
        if self.game_mode == 3:
            ...
            i = int(round((event.y - self.border) / self.pixels_per_grid_unit))
            j = int(round((event.x - self.border) / self.pixels_per_grid_unit))
    ...
    def draw_landmark(self, location, color):
        ...
        self.canvas.create_oval(self.border + j * self.pixels_per_grid_unit - 6,
                                self.border + i * self.pixels_per_grid_unit - 6,
                                self.border + j * self.pixels_per_grid_unit + 6,
                                self.border + i * self.pixels_per_grid_unit + 6,
                                outline=color,
                                fill=color)
